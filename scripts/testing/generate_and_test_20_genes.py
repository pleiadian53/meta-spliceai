#!/usr/bin/env python3
"""
Generate predictions for 20+ genes and validate score-shifting.

Step 1: Generate predictions using base_only mode
Step 2: Validate alignment using stratified analysis
"""
...
def generate_predictions_for_genes(gene_ids: List[str], mode: str = 'base_only'):
    """Generate predictions for a list of genes."""
    ...
    logger.info(f"\n{'='*80}")
    logger.info(f"STEP 1: GENERATING PREDICTIONS FOR {len(gene_ids)} GENES")
    logger.info(f"{'='*80}")
    ...
    if not model_path.exists():
        ...
        logger.info(f"   This is OK for base_only mode")
    ...
    for i, gene_id in enumerate(gene_ids, 1):
        ...
        try:
            ...
            config = EnhancedSelectiveInferenceConfig(
                ...
                inference_mode=mode,
                output_name='score_shifting_validation',
    ...
    logger.info(f"\n✅ Predictions generated for {len(successful_genes)}/{len(gene_ids)} genes")
    return successful_genes


def load_gtf_splice_sites(gene_id: str):
...
def analyze_position_offsets(predictions_df: pl.DataFrame, annotations_df: pl.DataFrame, 
    ...
    CRITICAL (2025-10-31): Uses splice-type-specific view columns for evaluation.
    - For donors: uses donor_score_donor_view (if available, else donor_score)
    - For acceptors: uses acceptor_score_acceptor_view (if available, else acceptor_score)
    
    This ensures we're using the correctly adjusted scores for each splice type.
    """
    results = {
    ...
    for splice_type in ['donor', 'acceptor']:
        ...
        # Use splice-type-specific view column if available
        view_col = f'{splice_type}_score_{splice_type}_view'
        if view_col in predictions_df.columns:
            score_col = view_col
            logger.debug(f"  Using {score_col} for {splice_type} evaluation")
        else:
            score_col = f'{splice_type}_score'
            logger.debug(f"  Using {score_col} for {splice_type} evaluation (no view column)")
...
def evaluate_gene(gene_info: Dict, mode: str = 'base_only'):
    ...
    logger.info(f"\n{'='*80}")
    logger.info(f"Testing {gene_id} ({gene_name}) - Strand {expected_strand}")
    logger.info(f"{'='*80}")
    
    try:
        # Load predictions (check actual output location)
        ...
        if not pred_file.exists():
            logger.warning(f"  ⚠️  No predictions file: {pred_file}")
        ...
        for splice_type in ['donor', 'acceptor']:
            ...
            if total > 0:
                ...
                if exact_pct >= 70:
                    ...
                    logger.info(f"      Status: ❌ Poor alignment")
        ...
        # Use splice-type-specific view columns if available
        donor_score_col = 'donor_score_donor_view' if 'donor_score_donor_view' in predictions_df.columns else 'donor_score'
        acceptor_score_col = 'acceptor_score_acceptor_view' if 'acceptor_score_acceptor_view' in predictions_df.columns else 'acceptor_score'
        
        pred_donors = set(predictions_df.filter(pl.col(donor_score_col) > 0.5)['position'].to_list())
        pred_acceptors = set(predictions_df.filter(pl.col(acceptor_score_col) > 0.5)['position'].to_list())
...
def main():
    """Main test function."""
    logger.info("="*80)
    logger.info("COMPREHENSIVE SCORE-SHIFTING VALIDATION: 20+ NEW GENES")
    logger.info("="*80)
    
    # Get test genes
    test_genes = get_20_test_genes()
    gene_ids = [g['gene_id'] for g in test_genes]
    
    logger.info(f"\nTesting {len(test_genes)} NEW genes:")
    logger.info(f"  + strand: {sum(1 for g in test_genes if g['strand'] == '+')} genes")
    logger.info(f"  - strand: {sum(1 for g in test_genes if g['strand'] == '-')} genes")
    
    # Step 1: Generate predictions
    try:
        generate_predictions_for_genes(gene_ids, mode='base_only')
    except Exception as e:
    ...
    # Step 2: Validate
    logger.info(f"\n{'='*80}")
    logger.info(f"STEP 2: VALIDATING PREDICTIONS")
    logger.info(f"{'='*80}")
    
    results = []
    for gene_info in test_genes:
        result = evaluate_gene(gene_info, mode='base_only')
    ...
    if successful:
        ...
        if avg_f1 >= 0.7 and len(successful) >= 15:
            ...
            logger.warning(f"⚠️  PARTIAL: {len(successful)} genes, avg F1: {avg_f1:.3f}")
    
    # Save results
    output_path = Path("results/score_shifting_20_genes_validation.json")
