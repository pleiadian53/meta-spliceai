...
check_conda_env() {
    # Initialize conda/mamba first
    if [ -f ~/miniforge3-new/bin/activate ]; then
        source ~/miniforge3-new/bin/activate
    elif [ -f ~/miniforge3/bin/activate ]; then
    ...
    if command -v mamba &> /dev/null; then
        ...
        echo "WARNING: Neither mamba nor conda found, skipping environment check"
    fi
}
...
builder_command() {
    ...
    if [ -f ~/miniforge3-new/bin/activate ]; then
        source ~/miniforge3-new/bin/activate surveyor
    elif [ -f ~/miniforge3/bin/activate ]; then
        source ~/miniforge3/bin/activate surveyor
    else
        source ~/.zshrc
        if command -v mamba &> /dev/null; then
...
run_with_nohup() {
    ...
    export N_GENES SUBSET_POLICY GENE_TYPES BATCH_SIZE BATCH_ROWS KMER_SIZES OUTPUT_DIR ADDITIONAL_GENES LOG_FILE
