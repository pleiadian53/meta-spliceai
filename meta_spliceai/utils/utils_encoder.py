...
def get_single_encoder(encoder_name: str, cat_cols: list):
    ...
    if encoder is None: 
        msg = f"Unrecognized encoder: {encoder_name}"
    ...
    def fit_transform(self, X: pd.DataFrame, y: np.array) -> pd.DataFrame:
        ...
        for encoder_name in self.encoders_names_tuple:
            for n_fold, (train_idx, val_idx) in enumerate(self.model_validation.split(X, y)):
                ...
                # NOTE: We can use the drop parameter to avoid the old index being added as a column
                ...
                # NOTE: if y_train has an index => could lead to potential error: `X` and `y` both have indexes, but they do not match.

                # transform validation part and get all non-numerical cols
                ...
                cols_representation[val_idx, :] += val_t / self.n_repeats # each fold contributes that this much of feature value
            ...
            self.storage.append(cols_representation) # foreach encoder, store its categorical variable coding ... (1)

        for df in self.storage:
            X = pd.concat([X, df], axis=1) # concatenate all encoders' categorical encoding
...
class MultipleEncoder(object):
    ...
    def __init__(self, cols: List[str], encoders_names_tuple=()):
        ...
        """
        self.cols = cols # categorical columns
        self.num_cols = None # to be inferred from X (any columns not categorical are assumed to be numerical)
        self.encoders_names_tuple = encoders_names_tuple
        self.encoders_dict = {}
    ...
    def fit_transform(self, X: pd.DataFrame, y: np.array) -> None:
        self.num_cols = [col for col in X.columns if col not in self.cols]
        self.storage = []
        nrows = X.shape[0] # test
        for encoder_name in self.encoders_names_tuple: # foreach encoder ...
        ...
        # print(f"... fit_transform: type(X): {type(X)}, shape(X): {X.shape}")
        for df in self.storage:
            # print(f"... + df type: {type(df)}, shape: {df.shape}")
            X = pd.concat([X, df], axis=1) # NOTE: sometimes gives wrong dimensionality, why? 
            # print(f'... => shape(X): {X.shape}')

        # remove all categorical columns as far as we have their representations
        X.drop(self.cols, axis=1, inplace=True)
...
def demo_basic():
    ...
    #
    print("\n> DoubleValidationEncoderNumerical")
    temp = df.copy()
