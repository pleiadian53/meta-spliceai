...
def gunzip(file):
    if os.path.exists(file):
        try:
            ...
            # subprocess.check_call(['sudo', 'gunzip', file])
            # NOTE: This may prompt for the sudo password when the function is called. 
            #       To allow the script to run gunzip command without password, open the sudoers file 
            #       with sudo visudo command and add the following line
            #       username ALL=(ALL:ALL) NOPASSWD: /bin/gunzip
...
def run_wget_and_gunzip(url, filepath):
    ...
    # Check if the file is a gzip file before trying to decompress it
    if filepath.endswith('.gz'):
        time.sleep(5)  # delay for 5 seconds to ensure the file is downloaded before unzipping
        gunzip(filepath)
    return
...
def remove_under(path_to_folder, verbose=0): 
    """
    (Recursively) Remove files and directories from under a directory but 
    ...
    for filename in os.listdir(folder):
        ...
        try:
            if os.path.isfile(file_path) or os.path.islink(file_path):
                ...
                shutil.rmtree(file_path) # recursively remove the directory including itself
                n_dirs += 1
        except Exception as e:
            print('Failed to delete %s. Reason: %s' % (file_path, e))
    if verbose: 
        print(f"[system] Removed {n_files} files and {n_dirs} folders.")
    states['n_files'] = n_files
    states['n_dirs'] = n_dirs
    return states
...
def index_file_name(output_dir, output_file, suffix="-"):
    assert os.path.exists(output_dir), f"Invalid output directory:\n{output_dir}\n"
    fn_base_ref, *ext = output_file.split(".")

    fn_new = output_file
    id_new = 0
    increment_index = add_index = False
    idx = []
    for f in os.listdir(output_dir):
        fn_base, *ext2 = f.split(".")
        fn_stem, *fn_id = fn_base.split(suffix) # does it have an experiment index, marked by the suffix? 
   
        if fn_stem == fn_base_ref: # `fn_stem`: file name without ID, without extension
            if (len(ext) > 0 and len(ext2) > 0) and (ext[0] == ext2[0]): # if file extension matches
                # indexed already? 
                if len(fn_id) > 0: 
                    idx.append(int(fn_id[0])) # find the max ID
                    increment_index = True
                else: 
                    add_index = True

    if increment_index or add_index: 
        if len(idx) > 0: 
            id_new = max(idx)+1
            fn_new = f"{fn_base_ref}{suffix}{id_new}"
        else: 
            assert not increment_index
            id_new = 1 # Todo: rename old file so that it has index 0? 
            fn_new = f"{fn_base_ref}{suffix}{id_new}"
        if len(ext) > 0: 
            fn_new = f"{fn_new}.{ext[0]}"
    else:
        # no-op
        pass
    return fn_new
...
def list_dir_with_constraints(source, ext_list=[], keywords=[], verbose=0, n_files=None):
    matches = []
    for root, dirnames, filenames in os.walk(source):
        matches_in_dir = []
        if verbose: print(f"> {root}")
        
        for filename in filenames:
            if len(ext_list) > 0:  # filter by file extensions
                if filename.endswith(ext_list):
                    matches_in_dir.append(os.path.join(root, filename))
            elif len(keywords) > 0: # filter by keywords
                for keyword in keywords: 
                    if filename.find(keyword) >= 0: 
                        matches_in_dir.append(os.path.join(root, filename))
        if matches_in_dir:
            matches.extend(matches_in_dir)

            if verbose: 
                for i, filename in enumerate(matches_in_dir): 
                    if n_files is not None and i >= n_files: break # display only n_files to avoid clusttering
                    print(f"... {filename}")
    return matches
...
def dumpclean(obj, n_sep=0):
    if type(obj) == dict:
        ...
        print (obj)
def display_dict_recursively(obj, n_sep=0): 
...
def sample_dict(adict, n_sample=10):
    """
    Returns a new dictionary with a sampled subset of the original dictionary's entries.
    
    Parameters:
    - adict (dict): The original dictionary to sample from.
    - n_sample (int): The number of entries to sample from the dictionary. If n_sample is greater
                      than the number of entries in adict, the entire dictionary is returned.
    
    Returns:
    - dict: A new dictionary containing the sampled entries.
    """
    import random
    # Ensure the number of samples does not exceed the number of available keys
    n_sample = min(n_sample, len(adict))
    # Directly sample from the dictionary's keys
    sampled_keys = random.sample(list(adict.keys()), n_sample)
    # Construct and return the sampled dictionary
    return {key: adict[key] for key in sampled_keys}
...
def demo_iter():
    ...
    display_dict_recursively(nested)
...
def demo_file_utility(): 
    ...
    output_file = "performance.tif"
    output_file_indexed = index_file_name(output_dir, output_file)
    
    print(f"> original: {output_file} -> indexed? {output_file_indexed}")
...
def demo_system():
    ...
    data_dir = get_data_dir('nmd')
    input_dir = data_dir # os.path.join(data_dir, 'data')
    cmd = f'ls -l {input_dir}'

    st, output = run_command(cmd)
    print(f"> status: {st}")
...
def demo_dir_walk(): 
    ...
    source_dir = get_data_dir('nmd', is_temp=True)
    print(f"> source dir:\n{source_dir}\n")

    ext_list = ['.csv', '.pkl', ]
    keywords = ['flanking_sc', 'partial_intron', ]
    matches = list_dir_with_constraints(source_dir, keywords=keywords, verbose=1, n_files=10)
    print(f"> Found n={len(matches)}")
...
def test():
    ...
    # print('=' * 80)
    # highlight(message='test message', symbol='*', border=2)
