...
Moved from ``scripts/patch_gene_type.py`` to keep all meta-model related
analysis / debugging utilities in ``meta_spliceai.splice_engine.meta_models.analysis``.

Usage
-----
conda run -n surveyor python -m meta_spliceai.splice_engine.meta_models.analysis.patch_gene_type <DATASET_ROOT> [--inplace]
...
# (Retained from original script – harmless inside package)
# ------------------------------------------------------------------
PROJECT_ROOT = pathlib.Path(__file__).resolve().parent.parent.parent  # …/meta_models
