"""
Approach B: Single-Pass Delta Predictor.

Key differences from Approach A (paired prediction):
- Input: alt_seq + variant_info (NO reference sequence needed)
- Target: SpliceVarDB-validated delta (ground truth filtering)
- Inference: Single forward pass (efficient)

The model predicts delta scores given:
1. The alternate sequence (with variant embedded)
2. Variant information (ref_base, alt_base)
3. Optionally: base model scores at the variant position

Target derivation:
- For Splice-altering variants: Use base model delta (trusted because SpliceVarDB confirms)
- For Normal variants: Zero delta (base model may be wrong, but we know the truth)
- For Low-frequency/Conflicting: Exclude from training

Usage:
    from meta_spliceai.splice_engine.meta_layer.models.approach_b_predictor import (
        ApproachBDeltaPredictor,
        create_approach_b_predictor
    )
    
    model = ApproachBDeltaPredictor(hidden_dim=128)
...
class ApproachBDeltaPredictor(nn.Module):
    ...
    include_base_scores : bool
        If True, also takes base model scores as input
    """
...
class ApproachBWithPositionAttention(nn.Module):
    """
    Approach B with position attention for interpretability.
    
    Adds an attention mechanism that shows which positions in the
    sequence are most important for the delta prediction.
    """
...
def create_approach_b_predictor(
    variant: str = 'basic',
    hidden_dim: int = 128,
    n_layers: int = 6,
    dropout: float = 0.1,
    ...
    """
    Factory function for Approach B predictors.
    ...
    dropout : float
        Dropout rate
    include_base_scores : bool
        Whether to include base model scores as input
    
    Returns
    -------
    nn.Module
        Approach B predictor
    """
    if variant == 'basic':
        return ApproachBDeltaPredictor(
            hidden_dim=hidden_dim,
            n_layers=n_layers,
            dropout=dropout,
        ...
        return ApproachBWithPositionAttention(
            hidden_dim=hidden_dim,
            n_layers=n_layers,
            dropout=dropout
        )
