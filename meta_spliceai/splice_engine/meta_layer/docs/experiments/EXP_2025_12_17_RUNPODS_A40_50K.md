...
**Status**: ✅ Complete - All 4 experiments finished
...
### 4. HyenaDNA Experiment ✅ COMPLETE - DISAPPOINTING
...
|--------|----------|------------|-------------------|
| Correlation | r > 0.50 | **r = 0.484** | ❌ -26% worse |
| ROC-AUC | > 0.65 | **0.562** | ❌ -8% worse |
| PR-AUC | > 0.75 | **0.692** | ❌ -5% worse |
| Detection @ 0.1 | - | 6.2% | - |
...
### ⚠️ Why HyenaDNA Underperformed

| Factor | Explanation |
|--------|-------------|
| **Frozen encoder** | HyenaDNA weights frozen → can't adapt to splice-specific patterns |
...
### Key Insight: Pre-training ≠ Automatic Win
...
1. **Unfreeze encoder gradually** (last few layers first) → ✅ Tested below
2. **Use longer context** (match HyenaDNA's 32K training)
3. **Add splice-specific heads** (separate donor/acceptor branches)
4. **Use larger HyenaDNA** (medium or large variants) → ✅ Tested below

---

### 5. HyenaDNA Fine-tuning Experiment ✅ COMPLETE - MINIMAL IMPROVEMENT
...
|--------|----------|------------|-----------|--------|
| Correlation | r > 0.55 | **r = 0.490** | +1.2% | ❌ -20% |
...
| **Task mismatch** | Pre-training (next-token) → delta regression is a large leap |
| **Architecture** | Single-pass pooled embeddings may lose position info |

### Key Conclusion: Task-Specific CNN Wins ✅

| Model | Params | Pre-training | Fine-tuning | Correlation |
|-------|--------|--------------|-------------|-------------|
| Gated CNN | ~1M | None | N/A | **r = 0.609** ✅ |
...
| **CNN Early Stopping** | **0.609** | **0.585** | **0.702** | 44/100 | 48 min | ✅ **BEST** |
| HyenaDNA Frozen | 0.484 | 0.562 | 0.692 | 18/50 | 36 min | ❌ Underperformed |
| HyenaDNA Fine-tuned | 0.490 | 0.600 | 0.692 | 37/50 | 43 min | ⚠️ Minimal gain |
...
**Conclusions**:
- ✅ Early stopping is essential for this task
- ✅ Task-specific CNN architecture is the best approach for validated delta prediction
- ❌ HyenaDNA (frozen or fine-tuned) underperforms task-specific CNN
- ❌ Pre-trained DNA models don't automatically help delta prediction
- 🎯 **Validated**: Task-specific architectures outperform general pre-training
...
### Completed ✅
1. ✅ Complete full 50K training → CNN r=0.609 (best)
2. ✅ Run HyenaDNA frozen experiment → r=0.484 (disappointing)
3. ✅ Run HyenaDNA fine-tuning experiment → r=0.490 (minimal gain)

### Recommended Next Steps 🎯

**HIGH PRIORITY**:
4. 🎯 **Meta-Recalibration**: Implement per-position splice score refinement ([L,3] output) to improve base model predictions upstream of delta computation
5. 🎯 **Multi-Step Framework**: Train binary classifier (Step 1: "Is this splice-altering?") - may achieve higher ROC-AUC than delta regression
6. 🎯 **Analyze predictions**: Examine failure modes - where does CNN succeed/fail?

**MEDIUM PRIORITY**:
7. ⏳ **Longer context for CNN**: Try 1K, 2K, 4K context windows (CNN can match HyenaDNA's context advantage)
8. ⏳ **Ensemble**: Combine CNN + HyenaDNA predictions
9. ⏳ **Effect-type specific models**: Separate models for donor vs acceptor effects

**EXPLORATORY**:
10. ⏳ **Alternative targets**: Use SpliceVarDB effect types directly instead of base model deltas
11. ⏳ **Cross-species**: Leverage conservation for better generalization
...
```
Epoch  1: train=0.0123, val=0.0112 ← Starting point
Epoch  5: train=0.0114, val=0.0108 ← Slow improvement
Epoch 10: train=0.0113, val=0.0107 ← Plateau phase
Epoch 15: train=0.0112, val=0.0106 ← Still plateaued
Epoch 20: train=0.0110, val=0.0106 ← Minor progress
Epoch 25: train=0.0105, val=0.0103 ← Breaking through
Epoch 30: train=0.0073, val=0.0101 ← Sharp drop (encoder adapting)
Epoch 35: train=0.0052, val=0.0113 ← OVERFITTING begins
...
|-----------|------------|--------------|----------------|
| HyenaDNA encoder | ~25M | 5e-6 | 0.1× |
| Projection layer | ~65K | 5e-5 | 1× |
| Variant embedding | ~4K | 5e-5 | 1× |
| Delta head | ~100K | 5e-5 | 1× |

### Why HyenaDNA May Need Different Approach

| Factor | Analysis |
|--------|----------|
| **Pre-training task mismatch** | HyenaDNA: next-token prediction on DNA → Our task: delta score regression |
| **Context utilization** | HyenaDNA trained on 160K context, but we feed 501bp → Massive context mismatch |
| **Feature granularity** | HyenaDNA learns general DNA grammar, we need splice-specific patterns |
| **Single-pass limitation** | HyenaDNA sees only alt sequence → Can't compare ref vs alt directly |
...
|----------|--------|-------------|-------|
| Gated CNN (scratch) | ~1M | **r=0.609** | ✅ Best |
| HyenaDNA (fine-tuned) | 25M | r=0.490 | 25× params, worse result |
...
- ❌ Frozen features + simple head
- ❌ Fine-tuning last 2 layers
- 🤔 May need: paired/siamese architecture, longer context, multi-task pre-training
...
**Results**:
- **ROC-AUC: 0.718** ✅ **Best result for variant triage!**
- **PR-AUC: 0.683**
...
**Results**:
- **ROC-AUC: 0.779** ⭐ Best overall!
- **PR-AUC: 0.791** ⭐ Best overall!
- F1 Score: 0.712
- Time: 100.9 minutes
...
### Experiment 2.5: HyenaDNA Binary (Fine-tuned)

**Hypothesis**: Unfreezing the last 2 HyenaDNA layers could improve binary classification further.

| Parameter | Value |
|-----------|-------|
| Model | HyenaDNA-small-32k + Classification Head |
| Encoder | **Unfrozen last 2 layers** |
| Trainable params | 3,543,553 (vs. 265K frozen) |
| Batch size | 16 |
| Epochs | 15 (early stop) |

**Training Progress**:
| Epoch | Val AUC | Train AUC | Patience |
|-------|---------|-----------|----------|
| 1 | 0.859 | 0.749 | 0/10 |
| 5 | **0.889** | 0.960 | 1/10 |
| 10 | 0.870 | 0.974 | 6/10 |

**Final Test Results** (held-out chr21-22):
- ROC-AUC: 0.667
- AP: 0.652
- F1 Score: 0.672
- Time: 34.7 minutes

**Critical Finding: Overfitting!** ⚠️

The fine-tuned model showed excellent validation performance (0.889) but poor test generalization (0.667). This is a classic overfitting pattern where the model memorizes training patterns rather than learning generalizable features.

**Why Frozen Works Better**:
1. Pre-trained representations are already optimized for DNA understanding
2. Fine-tuning with limited data (18K samples) causes overfitting
3. The classification head alone has enough capacity for this task
4. Frozen encoder preserves generalization to unseen chromosomes

### Phase 2 Summary

| Experiment | ROC-AUC | AP | Status |
|------------|---------|-----|--------|
| Binary Quick | 0.678 | 0.686 | ✅ Validates approach |
| Binary Full (CNN) | 0.718 | 0.683 | ✅ Strong baseline |
| **HyenaDNA Binary (Frozen)** | **0.779** | **0.791** | ⭐ **Best triage model** |
| HyenaDNA Binary (Fine-tuned) | 0.667 | 0.652 | ⚠️ Overfitting |
| Long Context 2K | 0.581 | 0.530 | ❌ No improvement |
...
| HyenaDNA Binary (Frozen) | ~14M | `20251218_005300/hyenadna_binary_hyenadna_binary_classifier.pt` |
| HyenaDNA Binary (Fine-tuned) | ~14M | `20251218_033637/hyenadna_binary_hyenadna_binary_classifier_(fine-tuned).pt` |
...
|----------|---------|--------|----------|
| **HyenaDNA Binary** | **0.779** | **0.791** | ⭐ **Best for triage** |
...
```
Variant → HyenaDNA Binary (ROC-AUC 0.779) → "Splice-altering"/"Normal"
                                          ↓ (if splice-altering)
                             Gated CNN Delta (r=0.42) → Δ scores
```

### 5. Model Performance Summary

| Model | Task | ROC-AUC | AP | Correlation | Best For |
|-------|------|---------|-----|-------------|----------|
| **HyenaDNA Binary (Frozen)** | Classification | **0.779** | **0.791** | N/A | ⭐ Triage |
...
| HyenaDNA Binary (Fine-tuned) | Classification | 0.667 | 0.652 | N/A | ⚠️ Overfits |

### 6. Lessons Learned

1. **Frozen pre-trained encoders generalize better** than fine-tuned ones with limited data
2. **Binary classification is easier** than delta regression for splice variant triage
3. **HyenaDNA's pre-trained representations** are valuable when kept frozen
4. **Fine-tuning requires more data** or stronger regularization to avoid overfitting
5. **Longer context (2K+)** does not help with current architectures

---

*Experiment log complete. Results saved for reproducibility.*
*Total compute time: ~9 hours on A40 GPU (~$6.20 total cost)*
