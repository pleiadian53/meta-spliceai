...
|------------|---------|--------|-------------|--------|
| Binary Quick | 0.678 | 0.686 | N/A | ✅ Validates approach |
| Binary Full | **0.718** | **0.683** | N/A | ✅ **Best triage model** |
| Long Context 2K | 0.581 | 0.530 | 0.131 | ❌ No improvement |
...
### 1. Binary Classification is Optimal for Variant Triage

The Multi-Step Step 1 binary classifier significantly outperforms delta regression:

| Approach | ROC-AUC | PR-AUC | Use Case |
|----------|---------|--------|----------|
| Binary Classifier | **0.718** | **0.683** | ✅ Variant triage |
| Delta Regression CNN | 0.585 | 0.487 | Magnitude estimation |

### 2. Standard Context is Sufficient

Longer context (2K bp) does not improve performance. The standard 401bp window captures the relevant splicing signals.

### 3. Task-Specific Architectures Win

| Approach | Params | ROC-AUC | Notes |
|----------|--------|---------|-------|
| Binary Classifier CNN | ~15M | **0.718** | ✅ Best for triage |
| Gated CNN Delta | ~1M | 0.585 | Good for delta estimation |
| HyenaDNA (fine-tuned) | 25M | 0.609 | Complex, not better |

### 4. Recommended Architecture

For variant classification pipeline:
```
Variant → Binary Classifier (ROC-AUC 0.718) → "Splice-altering"/"Normal"
                                           ↓
                             Delta Predictor (r=0.42) → Δ scores
```

---

*Experiment log complete. Results saved for reproducibility.*
*Total compute time: ~6 hours on A40 GPU (~$4.14 total cost)*
