...
def weighted_BCE_loss_on_negative(y_true, y_pred, from_logits=True, gamma=2, epsilon=1e-12):
    def weighted_BCE_loss(y_true, y_pred): 
        # e.g. y_true: (None,None,None,None)     y_pred: (None,512,512,1)
        y_pred = K.clip(y_pred, min_value=epsilon, max_value=1 - epsilon)
        weights = K.ones_like(y_pred)  # e.g. (None,512,512,1)
        weights = tf.where(y_pred < 0.5, gamma * weights, weights)
        # NOTE: Incor higher penalty on negative predictions so that it's as easy to predict negative

        out = keras.losses.binary_crossentropy(y_true, y_pred, from_logits=from_logits)  # (None,512,512)
        out = K.expand_dims(out, axis=-1) * weights  # (None,512,512,1)* (None,512,512,1)
        return K.mean(out)
    return weighted_BCE_loss

def weighted_binary_crossentropy(class_weight={0: 0.5, 1: 0.5}, from_logits=True):

    if class_weight is None or not class_weight: 
        class_weight = {0: 0.5, 1: 0.5}

    def weighted_binary_crossentropy_core(y_true, y_pred):

        # Original binary crossentropy (see losses.py):
        # K.mean(K.binary_crossentropy(y_true, y_pred), axis=-1)

        # Calculate the binary crossentropy
        b_ce = K.binary_crossentropy(y_true, y_pred, from_logits=from_logits)

        # Apply the weights
        one_weight = class_weight[1]
        zero_weight = class_weight[0]
    ...
    return weighted_binary_crossentropy_core

def weighted_BCE_loss_on_positive(y_true, y_pred, from_logits=True, gamma=2, epsilon=1e-12):
    """
    Memo
    ----
    1. Weighted BCE 
    ...
    def weighted_BCE_loss(y_true, y_pred): 
        # e.g. y_true: (None,None,None,None)     y_pred: (None,512,512,1)
        y_pred = K.clip(y_pred, min_value=epsilon, max_value=1 - epsilon)
        print(f"[BCE] type (y_pred): {y_pred}")
        weights = K.ones_like(y_pred)  # e.g. (None,512,512,1)
        weights = tf.where(y_pred >= 0.5, gamma * weights, weights)
        # NOTE: Incor higher penalty on positive predictions so that it's as easy to predict positive
...
def demo_binary_classifier(): 
    ...
    1. import error 
       ...
       (required by /home/bchiu/miniconda3/envs/nmd/lib/python3.9/site-packages/pandas/_libs/window/aggregations.cpython-39-x86_64-linux-gnu.so)
    
    2. Scikit-Learn Wrapper for Keras
    ...
    input_path = f"../data/demo/{input_file}"
    ...
    def create_baseline():
        ...
        # Compile model
        model.compile(loss=weighted_BCE_loss_on_positive, optimizer='adam', metrics=['accuracy']) # 'binary_crossentropy'
        return model

    # evaluate model with standardized dataset
    estimator = KerasClassifier(model=create_baseline, epochs=50, batch_size=5, verbose=1)
...
def test(): 
    ...
    # A complete example of fitting and evaluating a CNN model on the MNIST dataset
    # demo_cnn()
